...
# 6 sql查询时，MP与xml文件如何选择
判断依据：简单sql（两个或以下的查询条件）用MP，复杂sql用xml（不支持逻辑删除）。  
...
# 7 spring security自带login页面登录问题
## 7.1 问题描述
使用spring security自带页面登录时报错（浏览器页面：Error Page 404。浏览器控制台：302无响应数据）

## 7.2 解决
1. 无法访问security自带的登录界面：在security 的配置类filterChain(HttpSecurity http)中添加```.formLogin()```  
2. 解决1后出现 filterChain(HttpSecurity http)不放行我写的“/login”接口：
   - 在authorizeRequests()中配置 .antMatchers("/login") 放行登录接口——结果放行“/login”接口失败，因为配置后security只放行 get方式的“/login”接口  
   - 在webSecurityCustomizer()配置```web.ignoring().antMatchers("/login**");```，放行成功  
3. 解决1后出现 登录接口访问出错：自定义登录界面，并把头字段 Contant-Type 的值设置为application/x-www-form-urlencoded  
   - security自带的login界面，其POST请求头字段是  ```Contant-Type:application/x-www-form-urlencoded```——把表单信息放在url中。但是我给controller加的注解是@RestController，它接受的内容需要 放在请求体中，并且是json格式  
