...
### 4.4.1 解决
确认使用了该依赖的代码所在的项目 的pom文件导入了该依赖。然后去你的maven仓库里查看该依赖是否存在（进文件夹里看是否下载成功）。如果该依赖存在，那就在代码里手动import使用到的包（自行百度）；如果不存在：
1. 从中央仓库中下载依赖到maven：使用```mvn dependency:get -DremoteRepositories=https://repo1.maven.org/maven2/com/qiniu/qiniu-java-sdk/7.9.5/ -DgroupId=com.qiniu -DartifactId=qiniu-java-sdk -Dversion=7.9.5```。url去maven中央仓库或则阿里仓库（网站）查。
...
# 6 Spring-Security拦截OPTIONS请求的问题
